...
int	main(void)
{
	...
	char *comma = ft_strrchr(strrchr, ',');

	printf("%s\n", comma);

	char *attempt = ft_strrchr(strrchr, '@');
	...
	char *attempt_memchr = ft_memchr(for_memchr_name, '?', 20);

	printf("%s\n", attempt_memchr);

	char willSmitEmail[] = "willSmith_55@icloud.com";
	char *after_domain = ft_memchr(willSmitEmail, '@',
		ft_strlen(willSmitEmail));
	printf("%s\n", after_domain);
	...
	int mem_result = ft_memcmp(for_memcmp_text1, for_memcmp_text2,
	...
	char *haystcak_result1 = ft_strnstr(haystack, needle, 21);
	char *haystcak_result2 = strnstr(haystack2, needle2, 21);
	...
	char const set_text[] = " ";
	char *strtrim_result = ft_strtrim(s1_text, set_text);
